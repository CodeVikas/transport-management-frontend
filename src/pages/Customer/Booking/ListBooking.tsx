...
export default function BookingListPage() {
  ...
  const { customerBookings, statusCustomerBookings } = useSelector((state) => state.bookings);
  console.log("Bookings from state:", customerBookings);
  const bookingData = customerBookings?.items;
  ...
  useEffect(() => {
    if (statusCustomerBookings === "idle") {
  ...
  }, [dispatch, statusCustomerBookings, currentPage, bookingsPerPage, searchTerm]); // Add searchTerm to dependencies
  ...
  // Pagination Logic
  const totalPages = Math.ceil(customerBookings?.total / bookingsPerPage);

  const confirmDelete = () => {
    console.log("Deleting booking:", selectedBooking);
    if (selectedBooking) {
      dispatch(deleteBooking(selectedBooking)).then(() => {
        dispatch(fetchCustomerBookings({ skip: 0, limit: 10, search: "" }));
  ...
  return (
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <p className="text-sm text-gray-500">
              Total Bookings: {customerBookings.total}
            </p>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200 text-sm">
              {statusCustomerBookings === "loading" ? (
                <Loader colspan={5} />
    ...
    ${booking.tripStatus === "In Transit" ? "bg-blue-100 text-blue-800" : ""}
    ${
      booking.tripStatus === "Start Trip" ? "bg-purple-100 text-purple-800" : ""
    }
    ${booking.tripStatus === "Delivered" ? "bg-green-100 text-green-800" : ""}
  `}
                      >
                        {booking.tripStatus}
                      </span>
